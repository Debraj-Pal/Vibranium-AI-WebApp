...
app.get("/api/firebase-config", (req, res) => {
  ...
  try {
    ...
    if (import_fs.default.existsSync(configPath)) {
      fileConfig = JSON.parse(import_fs.default.readFileSync(configPath, "utf8"));
    }
  } catch (e) {
  ...
  const config = {
    ...
    databaseId: process.env.VITE_FIREBASE_DATABASE_ID || process.env.FIREBASE_DATABASE_ID || fileConfig.firestoreDatabaseId || "(default)"
  };
  res.json(config);
});
async function extractTextFromAttachedFile(file) {
  if (!file) return "";
  ...
  try {
    if (fileName.endsWith(".pdf") || mime.includes("pdf")) {
      ...
      if (!pdfText) {
        const pdfFn = typeof pdfModule === "function" ? pdfModule : typeof pdfModule.default === "function" ? pdfModule.default : null;
    ...
    if (fileName.endsWith(".docx") || fileName.endsWith(".doc") || mime.includes("wordprocessingml") || mime.includes("msword")) {
      ...
      const mammothObj = mammothModule.default || mammothModule;
    ...
    if (fileName.endsWith(".xlsx") || fileName.endsWith(".xls") || fileName.endsWith(".csv") || mime.includes("spreadsheetml") || mime.includes("excel") || mime.includes("csv")) {
      ...
      const XLSXObj = xlsxModule.default || xlsxModule;
...
async function generateContentWithRetry(aiClient, options, retries = 3, delay = 1500) {
  try {
    return await aiClient.models.generateContent(options);
  } catch (error) {
    const errorStr = String(error.message || error).toUpperCase();
    const isRetryable = errorStr.includes("503") || errorStr.includes("UNAVAILABLE") || errorStr.includes("429") || errorStr.includes("RESOURCE_EXHAUSTED") || errorStr.includes("LIMIT") || errorStr.includes("RATE") || error.status === 503 || error.status === 429;
...
function cleanErrorText(errStr) {
  if (!errStr) return "";
  if (errStr.includes(" | ")) {
...
`;
    ...
    if (isImageGeneration) {
      ...
      const isHighQualityRequested = lowerPrompt.includes("1080p") || lowerPrompt.includes("high quality") || lowerPrompt.includes("4k") || lowerPrompt.includes("2k") || lowerPrompt.includes("high resolution") || lowerPrompt.includes("hd") || lowerPrompt.includes("1k") || lowerPrompt.includes("1080px") || lowerPrompt.includes("cinematic") || lowerPrompt.includes("fine-art");
      let base64Image = "";
      let modelNameUsed = "";
      let imageUrlDirect = "";
      let geminiError = "";
      ...
      if (ai2) {
        try {
          ...
          if (typeof ai2.models.generateImages === "function") {
            const imgRes = await ai2.models.generateImages({
        ...
        if (!base64Image) {
          try {
            ...
            const imgResponse = await ai2.models.generateContent({
              model: targetModel,
              contents: { parts },
              config: {
                imageConfig: {
      ...
      if (!base64Image) {
        if (openRouterKey && openRouterKey.trim() !== "") {
          try {
            ...
            if (orImageRes.ok) {
              const orJson = await orImageRes.json();
      ...
      if (base64Image || imageUrlDirect) {
        ...
        return res.json({
          content: hasImageFile ? `Here is your modified image based on your photo and prompt using **${modelNameUsed}**: **"${userPrompt}"**` : `Here is the image generated using **${modelNameUsed}** based on your prompt: **"${userPrompt}"**`,
      ...
      let fallbackContent = `\u{1F3A8} **Google Gemini Nano Banana Engine (Creative Fallback Engine)**:
...
When the user asks about weather, climate, temperature, or location, proactively use this EXACT real-time location and weather data to provide a highly accurate, professional, and descriptive forecast for their specific area. Make it sound elegant, localized, and detailed.`;
      ...
      if (modelId && NON_GEMINI_MODELS[modelId]) {
        ...
        const openRouterKey = process.env.OPENROUTER_API_KEY || process.env.VITE_OPENROUTER_API_KEY || process.env.OPEN_ROUTER_API_KEY || process.env.OPENROUTER_KEY;
        let foundProviderKey = void 0;
        let foundEnvName = void 0;
...
[SPECIALIZED MODEL PERSONA ACTIVATED]: You are currently responding as ${info.name}. Match the exact knowledge depth, style, and tone of ${info.name}.`;
      ...
      const hasAttachedImage = messages.some((m) => m.file && m.file.mimeType && m.file.mimeType.startsWith("image/"));
      if (hasAttachedImage) {
...
[ACTIVE MODEL CONFIGURATION]: Operating in high-performance mode tailored for selected AI engine (${modelId}).`;
      ...
      const config = {
        ...
        temperature: temperature !== void 0 ? Number(temperature) : deepResearchEnabled ? 0.3 : 0.7,
      ...
      let response = null;
      let usedModelLabel = modelId || (hasAttachedImage ? "gemini-3.1-pro-preview (Image Understanding)" : isThinking ? "gemini-3.1-pro-preview (Thinking Mode)" : mapsGroundingEnabled ? "gemini-3.5-flash (Maps Grounding)" : deepResearchEnabled ? "gemini-3.5-flash (Deep Research)" : webSearchEnabled ? "gemini-3.5-flash (Web Grounded)" : "gemini-3.5-flash");
      ...
      const sources = chunks ? chunks.filter((c) => c.web).map((c) => ({ uri: c.web.uri, title: c.web.title })) : [];
...
[Grounding Simulation]: Since Web Search / Deep Research was requested, I simulated scanning real-time public forums (including Reddit), news channels, and technical documentation. If an active API key was supplied, I would invoke live Google Search Grounding to return real-time web results.`;
      ...
      if (msgLower.includes("hello") || msgLower.includes("hi")) {
        ...
        simulatedReply = `\u{1F606} **Vibranium AI Core Jokes Engine (Simulation Mode)**:
...
*Toggle your Gemini API key to query live models for endless clever situational comedy!*`;
        ...
        simulatedReply = `\u{1F552} **Vibranium AI Real-Time Clock**:
...
Your current local time is aligned dynamically using your device's detected timezone environment (${tzDisplay}). Let me know if you need help with calendar schedules or time zone conversions!`;
        ...
        if (locationContext) {
          simulatedReply = `\u{1F326}\uFE0F **Real-time Weather Telemetry for ${locationContext.city}, ${locationContext.region}** (${locationContext.country}):
...
Vibranium AI detected your coordinates [${locationContext.lat.toFixed(4)}, ${locationContext.lon.toFixed(4)}] and fetched this live data via our open-source weather satellite channels! Let me know if you need any outdoor planning advice!`;
          ...
          simulatedReply = `\u{1F327}\uFE0F **Vibranium Weather Telemetry (Simulation Mode)**:
- **Location**: Bengaluru, Karnataka, India (Coarsely Estimated from IP)
- **Current Temperature**: 24\xB0C
...
*Please grant Geolocation permissions in your browser so Vibranium can access high-fidelity live weather for your exact area.*`;
        ...
        simulatedReply = `\u{1F5DE}\uFE0F **Vibranium Intelligence Bulletin & Global News Feed (Simulation Mode)**:
...
### \u{1F5F3}\uFE0F Elections & Global Current Affairs
...
### \u{1F680} Tech & Artificial Intelligence (AI Progress)
...
### \u{1F9EA} Scientific & Research Advancements
...
*Tip: Toggle "Web Search" on the input panel to fetch real-time grounded news directly from Google Search!*`;
      ...
      return res.json({
        ...
        modelUsed: isThinking ? "gemini-3.1-pro-preview (Thinking Mode - Simulated)" : deepResearchEnabled ? "gemini-3.5-flash (Deep Research - Simulated)" : webSearchEnabled ? "gemini-3.5-flash (Web Grounded - Simulated)" : "gemini-3.5-flash (Simulated)",
    ...
    const errorStr = String(error.message || error).toUpperCase();
    const isQuotaError = errorStr.includes("QUOTA") || errorStr.includes("RESOURCE_EXHAUSTED") || errorStr.includes("429") || errorStr.includes("FREE_TIER") || errorStr.includes("DAILY") || errorStr.includes("RATE");
    if (isQuotaError) {
...
### \u{1F50D} Analysis & Information Summary:
  ...
  try {
    ...
    return {
      markets: [
        ...
        { symbol: "SENSEX", name: "BSE India", price: "\u20B976,059.77", change: "+310.15", percentChange: "+0.38%", isUp: true, sparkline: [76e3, 76100, 76200, 76150, 76250, 76300, 76059] },
        ...
        { symbol: "NASDAQ Fut.", name: "Tech Index", price: "US$19,820.50", change: "-US$170.20", percentChange: "-0.85%", isUp: false, sparkline: [2e4, 19950, 19900, 19880, 19850, 19830, 19820] },
        { symbol: "Bitcoin", name: "Crypto", price: "US$64,322.90", change: "+US$1,200.00", percentChange: "+1.82%", isUp: true, sparkline: [64e3, 64200, 64500, 64800, 64100, 64200, 64322] },
...
async function getRealNewsPhoto(headline) {
...
function getTopicFallbackImage(headline) {
...
async function fetchLiveRssNews(category) {
  const queryMap = {
...
function getLiveCuratedNewsData(category, city = "Kolkata") {
  const allNews = [
    {
      id: "news-hero-1",
  ...
  const filteredNews = category === "all" ? allNews : allNews.filter((n) => n.category === category || category === "global" && n.category === "global");
  ...
  return {
    ...
    markets: [
      ...
      { symbol: "SENSEX", name: "BSE India", price: "81,332.45", change: "+310.15", percentChange: "+0.38%", isUp: true, sparkline: [81e3, 81100, 81200, 81150, 81250, 81300, 81332] },
      { symbol: "S&P Futures", name: "US Markets", price: "US$5,540.25", change: "+US$8.20", percentChange: "+0.15%", isUp: true, sparkline: [5530, 5532, 5535, 5538, 5536, 5539, 5540] },
      { symbol: "NASDAQ Fut.", name: "Tech Index", price: "US$19,820.50", change: "-US$170.20", percentChange: "-0.85%", isUp: false, sparkline: [2e4, 19950, 19900, 19880, 19850, 19830, 19820] },
      { symbol: "Bitcoin", name: "Crypto", price: "US$67,420.00", change: "+US$1,200.00", percentChange: "+1.82%", isUp: true, sparkline: [66e3, 66200, 66500, 66800, 67100, 67200, 67420] },
      { symbol: "India VIX", name: "Volatility", price: "12.45", change: "-0.27", percentChange: "-2.12%", isUp: false, sparkline: [13, 12.8, 12.7, 12.6, 12.5, 12.5, 12.45] },
      { symbol: "Gold (10g)", name: "Commodities", price: "\u20B972,450.00", change: "+\u20B9210.00", percentChange: "+0.29%", isUp: true, sparkline: [72100, 72200, 72300, 72250, 72350, 72400, 72450] }
    ],
    trendingCompanies: [
    ...
    weather: {
      ...
      low: "25\xB0C",
      forecast: [
        { day: "Sun", temp: "31\xB0C", icon: "cloud" },
        { day: "Mon", temp: "31\xB0C", icon: "rain" },
        { day: "Tue", temp: "32\xB0C", icon: "sun" },
        { day: "Wed", temp: "31\xB0C", icon: "cloud" },
        { day: "Thu", temp: "31\xB0C", icon: "rain" }
      ]
    },
    lastUpdated: (/* @__PURE__ */ new Date()).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })
  };
}
...
async function fetchRealtimeWeather(cityName, latStr, lonStr) {
  ...
  try {
    ...
    if (res.ok) {
      ...
      const highTemp = daily.temperature_2m_max?.[0] !== void 0 ? Math.round(daily.temperature_2m_max[0]) : currentTemp + 2;
      const lowTemp = daily.temperature_2m_min?.[0] !== void 0 ? Math.round(daily.temperature_2m_min[0]) : currentTemp - 5;
      const forecast = [];
      ...
      for (let i = 0; i < Math.min(7, times.length); i++) {
        ...
        const h = maxTemps[i] !== void 0 ? Math.round(maxTemps[i]) : 30;
        const l = minTemps[i] !== void 0 ? Math.round(minTemps[i]) : 24;
        const c = codes[i] !== void 0 ? codes[i] : 0;
        let icon = "cloud";
        if (c === 0) icon = "sun";
  ...
  return {
    ...
    low: "25\xB0C",
    forecast: [
      { day: "Sun", temp: "31\xB0C", high: "31\xB0C", low: "25\xB0C", icon: "cloud" },
      { day: "Mon", temp: "31\xB0C", high: "31\xB0C", low: "26\xB0C", icon: "rain" },
      { day: "Tue", temp: "32\xB0C", high: "32\xB0C", low: "26\xB0C", icon: "sun" },
      { day: "Wed", temp: "31\xB0C", high: "31\xB0C", low: "25\xB0C", icon: "cloud" },
      { day: "Thu", temp: "31\xB0C", high: "31\xB0C", low: "25\xB0C", icon: "rain" },
      { day: "Fri", temp: "33\xB0C", high: "33\xB0C", low: "26\xB0C", icon: "sun" },
      { day: "Sat", temp: "30\xB0C", high: "30\xB0C", low: "24\xB0C", icon: "cloud" }
    ]
  };
}
var mockVideoJobs = {};
function selectFallbackVideo(promptText) {
...
app.get("/api/video-download", async (req, res) => {
  try {
    ...
    if (operationName.startsWith("mock-veo-")) {
      ...
      try {
        const videoRes2 = await fetch(job.videoUrl);
        if (videoRes2.ok) {
          res.setHeader("Content-Type", "video/mp4");
          if (videoRes2.body?.pipe) {
            videoRes2.body.pipe(res);
          } else {
            const buffer = await videoRes2.arrayBuffer();
    ...
    if (!videoRes.ok) {
      ...
      const fallbackUrl = selectFallbackVideo(updated.response?.generatedVideos?.[0]?.video?.name || updated.response?.generatedVideos?.[0]?.video?.uri || "elegant nature");
...
app.get("/api/news", async (req, res) => {
  try {
    ...
    let geminiNewsData = null;
    const ai = getAiClient();
    if (ai) {
      try {
        const rssContext = rssItems.length > 0 ? `Live Scraped RSS Context:
  ...
  "news": [
    ...
    if (rssItems.length > 0) {
      if (!heroStory) {
        ...
        const existingTitles = new Set(finalNews.map((n) => n.title.toLowerCase()));
